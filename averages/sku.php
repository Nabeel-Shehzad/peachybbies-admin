            ...
            <div class="col-12 col-md-8 col-sm-12">
                <table id="employee_data" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    if (isset($_POST['submit'])) {
                        $username = $_POST['user'];
                        $sql = "SELECT employee.first_name,employee.last_name,slime.slime_name, (SUM(actual_quota) / SUM(HOUR(TIMEDIFF(TIME(end_time), TIME(start_time))))) AS Average
                                FROM data 
                                JOIN employee ON employee.id = data.username 
                                JOIN slime On slime.id = data.sku_packed 
                                WHERE '$username' = employee.first_name
                                GROUP BY slime.slime_name";
                        $result = $conn->query($sql);
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ?>
